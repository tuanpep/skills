...
description: >
  ...
  Build smallest clear solution now; grow only with real pressure.
---
...
- ship fast, maintainable impl
- simplify abstraction-heavy code
...
4. YAGNI.
5. Readable > clever.
6. Refactor on real pressure.

## Hard rules
1. Solve current req only.
2. Follow repo conventions.
3. No new layer without immediate proof.
4. No generic util for one caller.
5. Avoid pass-through wrappers.
6. Split fn only when readability/testability improves.
7. Use explicit domain names.
8. Plain control flow + data.
9. Add deps only when built-ins/current deps fail.
10. Add config/env only for real behavior variance now.
11. Optimize only with measurement.
12. Tests cover changed behavior + relevant failure path.

## Rule of three
1st use: direct code.
2nd use: small duplication OK.
3rd stable repeat: extract minimal abstraction.

## Simplicity checklist
- delete code, still meets req?
- remove layer?
- touch fewer files?
- each branch tied to current req?
- mid-level engineer reads fast?
- code health same or better?
Any yes -> simplify more.

## Avoid
- speculative extensibility
- one-caller generic helper
- zero-behavior wrappers
- unmeasured perf complexity
- config explosion
- hidden side effects/magic

## Workflow
1. Restate req in one sentence.
2. Pick shortest clear path in existing structure.
3. Add/update tests (happy + edge/failure).
4. Remove dead code/wrappers.
...
## Review gate (block if true)
- abstraction has one consumer
- extensibility point has no near-term req
- new dep saves only few simple lines
- new config has one practical value
- optimization has no bottleneck evidence

## Response behavior
- give one preferred approach
- alternatives max 2, short
- call overengineering risk explicitly
- state deferred complexity as trigger rule
---
name: caveman-code
description: >
  Practical coding defaults. Correctness + code health first. KISS/YAGNI.
